...
class SupabaseService:
    ...
    def submit_game_stats(self, stats: Dict):
        ...
        try:
            ...
            for question in stats["questions"]:
                ...
                self.supabase.table("question_attempts").insert(question_attempt).execute()
    ...
    def get_student_stats(self, student_id: str) -> Dict:
        ...
        try:
            # Get game sessions
            sessions = self.supabase.table("game_sessions")\
                .select("*")\
            ...
            total_correct = sum(session["correctly_answered"] for session in sessions.data)
            total_questions = sum(session["total_answered"] for session in sessions.data)
            ...
            return {
                ...
                "average_time_per_session": total_time / total_sessions if total_sessions > 0 else 0,
    ...
    def get_class_stats(self) -> Dict:
        ...
        try:
            ...
            # Calculate statistics
            total_students = len(set(session["student_id"] for session in sessions.data))
            total_sessions = len(sessions.data)
            total_correct = sum(session["correctly_answered"] for session in sessions.data)
            total_questions = sum(session["total_answered"] for session in sessions.data)
            total_time = sum(session["total_time"] for session in sessions.data)
            ...
            return {
                ...
                "total_correct_answers": total_correct,
                "total_questions_attempted": total_questions,
                "class_average_accuracy": total_correct / total_questions if total_questions > 0 else 0,
                "average_time_per_session": total_time / total_sessions if total_sessions > 0 else 0,
                "sessions_by_game_type": self._count_by_game_type(sessions.data),
                "sessions_by_level": self._count_by_level(sessions.data)
            }
    ...
    def initialize_connection(self):
        ...
        """
        # Implementation of initialize_connection method
        pass
