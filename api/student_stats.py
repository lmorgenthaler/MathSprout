...
def get_student_stats():
    ...
    try:
        ...
        sessions = (
            db.table("game_sessions").select("*").eq("student_id", student_id).execute()
        )
...
def get_level_progress(db, student_id: str, game_type: str) -> List[Dict]:
    ...
    try:
        ...
        level_criteria = (
            db.table("level_criteria").select("*").eq("game_type", game_type).execute()
        )
        ...
        for level in range(1, 4):  # Assuming 3 levels per game type
            level_data = next(
                (s for s in level_stats.data if s["level"] == level), None
            )
            criteria = next(
                (c for c in level_criteria.data if c["level"] == level), None
            )
            ...
            if level_data:
                progress = min(
                    100, (level_data["high_score"] / criteria["required_score"]) * 100
                )
...
def check_level_unlock():
    ...
    try:
        ...
        unlocked = best_score.data["high_score"] >= (
            criteria.data["required_score"] * 0.7
        )
